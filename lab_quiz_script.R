...
analytic.data.practice <- read_csv("mmr_labquiz_data.csv")
...
sd.x <- sd(analytic.data.practice$x.centered, na.rm=TRUE)
sd.z<-sd(analytic.data.practice$z.centered, na.rm=TRUE)
...
my.plot <- my.plot + annotate("text", x = -1, y = 75, label = "+1 SD Preparation")
my.plot <- my.plot + annotate("text", x = 1, y = 30, label = "-1 SD Preparation")
